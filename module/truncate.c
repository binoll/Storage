...
int truncate_file_no_zero(const char *path, loff_t new_size) {
    ...
    if (new_size < inode->i_size) {
        ...
        printk(KERN_ERR "New size is greater than or equal to current size. Only truncation is supported.\n");
        error = -EINVAL;
    }
...
MODULE_DESCRIPTION("A kernel module to truncate a file to a specified length without zeroing bytes");
